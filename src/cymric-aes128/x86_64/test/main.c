...
int main(void) {
    ...
    for(size_t i = 0; i < outlen; i++)
      printf("%02x", ctext[i]);
    printf("\n");
    ret = cymric1_dec(ptext_bis, &outlen, key, nonce, 12, ctext, outlen, ad, 3, &aes_ctx);
    ...
    for(size_t i = 0; i < outlen; i++)
      printf("%02x", ctext[i]);
    printf("\n");
    ret = cymric1_dec(ptext_bis, &outlen, key, nonce, 8, ctext, outlen, ad, 4, &aes_ctx);
    ...
    for(size_t i = 0; i < outlen; i++)
      printf("%02x", ctext[i]);
    printf("\n");
    ret = cymric2_dec(ptext_bis, &outlen, key, nonce, 12, ctext, outlen, ad, 3, &aes_ctx);
