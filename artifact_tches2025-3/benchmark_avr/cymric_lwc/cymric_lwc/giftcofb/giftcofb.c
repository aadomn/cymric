...
u32 ad_len, const u8* in, u32 in_len, const int encrypting) {
	...
	mygift128_kexp(rkey, key);
	mygift128_enc(Y, nonce, rkey);
	offset[0] = ((u32*)Y)[0];
	offset[1] = ((u32*)Y)[1];
...
****************************************************************************/
int giftcofb_encrypt(unsigned char* c, unsigned long long* clen,
...
* API required by the NIST for the LWC competition.
****************************************************************************/
int giftcofb_decrypt(unsigned char* m, unsigned long long *mlen,
