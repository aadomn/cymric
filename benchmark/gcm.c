	...
	const unsigned char* adata, unsigned int adata_len)
{
	 ...
	 * AES_ENC_BLOCK(H, H, rkeys);
	 */

	AES_ENC_BLOCK(ctext + ptext_len, J0, rkeys);

	/* C = GCTR_K(inc_32(J_0), P) */
	if (ptext_len != 0) {
		J0[BLOCKBYTES - 1] += 1;
		AES_ENC_BLOCK(J0, J0, rkeys);
  		//aes(Mode, &aes_struct, key, J0, J0, key, key);
	...
	const unsigned char* adata, unsigned int adata_len)
{
	...
	//memset(H, 0, BLOCKBYTES);
	//AES_ENC_BLOCKS(H, H, H, H, rkeys);
	memcpy(J0 + BLOCKBYTES, J0, BLOCKBYTES);
	J0[2*BLOCKBYTES - 1] += 1;
	AES_ENC_BLOCKS(ctext + ptext_len, J0, J0, J0 + BLOCKBYTES, rkeys);
